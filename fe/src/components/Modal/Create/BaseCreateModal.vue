<template>
  <div>
    <primary-button class="text-medium h-8 font-medium" @click.enter="isModalVisible = true">
      <template #label>
        {{ modalTitle }}
      </template>
    </primary-button>
    <a-modal v-model:open="isModalVisible" :title="modalTitle" centered class="">
      ...
      <template #footer>
        <div class="mt-8 flex justify-end gap-1">
          ...
          <primary-button
            ...
            ><template #label>
              <a-spin :indicator="loadingSpinner" v-show="confirmLoading" /> Confirm
            </template>
          </primary-button>
        </div>
      </template>
...
const isModalVisible = defineModel('isModalVisible', Boolean)
...
watch(isModalVisible, (value) => {
  if (value) {
    props.fieldsInfo.forEach((field) => {
      props.fieldsData[field.key] = ''
    })
